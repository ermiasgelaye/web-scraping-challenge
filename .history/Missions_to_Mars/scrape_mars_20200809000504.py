...
def scrape():
            ...
            latest_text = results[0].select('.article_teaser_body')[
                0].get_text()
            latest_title
            latest_text
            ...
            featured_image_url = 'https://www.jpl.nasa.gov' + \
                RE.findall(results[0].select('article')
                           [0]["style"])[0].strip("'")
            # featured_image_url
            ...
            mars_weather = soup.select(
                "li.js-stream-item")[0].select("p.TweetTextSize")[0].text
            mars_weather
            ...
            col1 = [a.get_text()
                    for a in soup.select("table")[0].select(".column-1")]
            col1 = [a.rstrip('[:]') for a in col1]
            col2 = [a.get_text()
                    for a in soup.select("table")[0].select(".column-2")]
            col2 = [a.rstrip('\n') for a in col2]
            ...
            facts_df = tables[0].rename(
                columns={0: " ", 1: "Value"}).set_index(" ")
            facts_df
            ...
            for item in soup.select('a.itemLink'):
                ...
                img_url.append("https://astrogeology.usgs.gov" +
                               soup1.select('img.wide-image')[0]['src'])

            hemisphere_image_urls = [
                {'title': title[i], 'img_url':img_url[i]} for i in range(len(title))]
            hemisphere_image_urls
